...
const AutoComplete = (props: Props) => {
  ...
  const defaultSelection = useMemo(
    () => {
      ...
      return selections.filter((item) => currentValue.includes(item.value))
    },
    [getValues, name, selections]
  )
...
const SelectionDropDownList = ({
  ...
  return (
    ...
    >
      {selections.length === 0 ? (
        <Box padding={3}>
          <Text color="black"> There are no Record Found.</Text>
        </Box>
      ) : (
        selections.map((selection) => (
          ...
          >
            <Text color="black">{selection.title}</Text>
          </ListItem>
        ))
      )}
    </List>
  ...
  return (
    <>
      {selectedItems.map((item) => (
        <Tag key={item.id} size="md" variant="solid" bgColor="gray.300" color="black" mr={1}>
          {item.title}
        </Tag>
      ))}
    </>
